  ...
  return (
    <div className="mb-20">
      {loading ? (
        ...
        <div className="bg-black backdrop-blur-lg border border-white/10 rounded-2xl pt-6 pb-10 px-6 sm:px-10 md:px-16 text-white font-sans leading-relaxed shadow-[0_8px_30px_rgb(0,0,0,0.3)] max-w-3xl mx-auto space-y-10">
          {" "}
          {/* Strengths */}
          ...
          {strengths.length > 0 && (
            <div className="space-y-4">
              ...
              {strengths.map((line, index) => (
                <div
                  key={`strength-${index}`}
                  className="bg-[#0f131a] text-left p-3 rounded-lg border border-green-600 text-green-400 text-sm"
                >
                  {line}
                </div>
              ))}
          ...
          {problems.length > 0 && (
            <div className="space-y-4">
              <h3 className="text-md font-semibold mb-2">Problems ❌</h3>
            ...
            <div className="mt-6">
              ...
              <ul className="space-y-4 text-sm text-gray-300 p-0">
                {recommendations.map((rec, index) => (
                  <div
                    key={`rec-${index}`}
                    className="bg-[#0f131a] text-left p-3 rounded-lg text-sm"
                  >
                    {"➡ " + rec}
                  </div>
                ))}
